...
export const assemblyPhases: Phase[] = [
  // ─── Level 0 ─────────────────────────────────────────────────────────────
  {
    ...
    intro:
      "Welcome — and if you have never written a single line of code before, that is fine; we start from absolute zero here.\n\n**What is the CPU?** The CPU is the chip inside your computer that actually *does* things — it adds numbers, copies values around, and follows instructions one at a time. **Assembly** is the most direct, human-readable way to tell the CPU what to do. Every other language (Python, JavaScript, C…) is eventually translated down into instructions like the ones you are about to write. So you are learning the bedrock.\n\n**What is a register?** Picture the CPU as having a few tiny labelled boxes built right into it, each able to hold one number. These boxes are called *registers*. They are the fastest storage in the whole machine. One of them is named `rax`. When we say \"put 5 in rax\" we just mean \"write the number 5 into the box labelled rax.\"\n\n**What is an instruction?** An instruction is one small command for the CPU, written on its own line, like `mov rax, 5` (\"put 5 in the rax box\"). A *program* is simply a list of these instructions that the CPU runs top to bottom, one after another — like a recipe.\n\nIn this course we use **NASM syntax**, where a copy instruction is written `destination, source` — the box you are writing *into* comes first, the value coming *from* comes second. To follow along on your own machine you'll install the NASM assembler and the GNU linker, then turn your text file into a runnable program with `nasm -f elf64 main.asm -o main.o && ld main.o -o main`. The checks in this app run on a small educational register VM that simulates a handful of registers and instructions and adds a teaching-only `print` pseudo-op (a fake instruction that exists only to help you learn) — so you can experiment instantly, while we teach the full real instruction set (syscalls, addressing modes, calling conventions) through reading exercises.",
    topics: [
    ...
    checks: [
      {
        ...
        explanation:
          "Let's read this first program token by token, top to bottom, the way the CPU does. Each line is one *instruction* — one small command.\n\n```\n; main.asm — first program on the register VM   <- a comment (CPU ignores it)\nmov rax, 1        ; load 1 into the rax register   <- put the number 1 in box rax\nprint \"Hello, World!\"                              <- show that text on screen\nhlt               ; stop the program              <- halt: stop here\n```\n\n**Line 1 — `; main.asm …`**\n- *What it means:* anything after a semicolon `;` is a **comment** — a note for humans. The CPU skips it entirely.\n- *Why it's there:* to remind a reader what the file is. It does no work.\n- *If removed:* nothing changes; the program runs identically. Comments are purely for people.\n- *At runtime:* nothing — comments never become CPU instructions.\n\n**Line 2 — `mov rax, 1`** (this is the real heart of the program)\n- `mov` *What it means:* short for \"move,\" but it really **copies**. It is a verb telling the CPU \"copy a value into a box.\"\n- `rax` *What it means:* the name of one of the CPU's tiny storage boxes (a **register**). This is the *destination* — the box we are writing into. In NASM the destination always comes first.\n- `1` *What it means:* the literal number one. A fixed value written right in the code like this is called an **immediate**. This is the *source* — what we copy from.\n- *Why it's there:* it loads the value 1 into rax so the box holds a known number. (\"move\" is a slightly misleading name: the 1 is *copied* in; nothing is taken away from anywhere.)\n- *If removed:* the program would still print the greeting, because `print` below uses fixed text, not rax — but rax would be left holding whatever junk happened to be there. We include it to demonstrate the most fundamental instruction in all of assembly.\n- *At runtime:* immediately after this line executes, the rax box literally contains the number 1.\n\n**Line 3 — `print \"Hello, World!\"`**\n- `print` *What it means:* a teaching-only **pseudo-op** — a pretend instruction that exists in this course's learning VM, not in a real CPU. It writes text to the screen (standard output).\n- `\"Hello, World!\"` *What it means:* a **string** — a piece of literal text. The double quotes mark where the text starts and ends; they are not printed themselves.\n- *Why it's there:* to produce visible output so you can confirm the program ran.\n- *If removed:* nothing would be printed and the check would fail. On a real Linux machine you would replace this with the `write` syscall (covered later); `print` is just an easy stand-in.\n- *At runtime:* the characters `Hello, World!` appear on screen. (Note it prints fixed text, not the contents of rax.)\n\n**Line 4 — `hlt`**\n- *What it means:* short for \"halt\" — stop executing.\n- *Why it's there:* to mark a clean end. Without an explicit stop, the CPU would keep reading whatever bytes come next and behave unpredictably.\n- *If removed:* in this tiny VM there is nothing after it, but as a habit you always tell the CPU when to stop.\n- *At runtime:* the program ends here.\n\n**One more concept: labels.** You'll see them soon. A **label** is just a name you stick on a line, written like `start:` (a word followed by a colon), so you can refer to that spot later — think of it as a bookmark in the list of instructions. There are no labels in this first program, but keep the idea in your pocket.",
      },
      {
        kind: 'mcq',
        id: 'assembly-0-mcq-1',
  ...
  // ─── Level 1 ─────────────────────────────────────────────────────────────
  {
    ...
    intro:
      "Let's slow down and build up the bedrock idea this whole course rests on: the **register**. (If you read Level 0, this is the same box concept, now in full.)\n\n**What is a register?** A register is a tiny storage slot built directly into the CPU chip. Think of it as a small labelled box that holds exactly one number at a time. Writing a new number into a box simply overwrites whatever was there before — like erasing a sticky note and writing a fresh value. Registers are the *fastest* storage in the entire computer; the CPU can read and write them instantly. (Your computer also has much larger but slower memory, called RAM, which we'll meet later.)\n\n**The main registers.** x86-64 gives you sixteen of these 64-bit boxes, called *general-purpose registers* because you may use them for almost anything. Their names are: `rax`, `rbx`, `rcx`, `rdx`, `rsi`, `rdi`, `rbp`, `rsp`, and `r8` through `r15`. The names are historical — for now just treat them as box #1, box #2, and so on. We mostly use `rax`, `rbx`, and `rcx` in this phase. (Two of them, `rsp` and `rbp`, have special jobs you'll learn about later, so leave those alone for now.)\n\n**\"An instruction operates on registers\"** simply means: each command you write reads from and/or writes to these boxes. For example `mov rbx, rax` reads the number in the `rax` box and copies it into the `rbx` box. The instruction doesn't act on thin air — it acts *on* the named registers. The verb `mov` (\"move,\" really \"copy\") is the workhorse: it copies a value into a register, either a fixed number (an *immediate*, like `mov rax, 42`) or the contents of another register (like `mov rbx, rax`).\n\n**A program is a top-to-bottom list.** Your code is just a sequence of instructions, one per line. The CPU starts at the top and runs them in order, each one changing the contents of some boxes, until it reaches the end. There is no hidden magic — to know what a register holds at any point, you just \"play computer\" and walk the lines from the top.\n\n**Same box, different views.** Each 64-bit register has narrower nicknames (*aliases*) that look at the *same* underlying box but only the lower portion of it: `eax` is the low 32 bits of `rax`, `ax` the low 16 bits, `al` the low 8 bits. They are not separate boxes — change one and you change the overlapping part of `rax` too. By the end of this phase you'll be able to predict the contents of any register after a sequence of `mov`s, and know which aliases overlap.\n\nLocally, build a tiny program that loads several constants into registers and inspect them in GDB (a debugger) with `info registers` after setting a breakpoint at `_start`. On the VM here you'll use `print rax` to read a register's value.",
    video: {
